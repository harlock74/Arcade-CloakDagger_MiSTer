...
- `Documents/cloak.pdf`
- Separated sheet PDFs in `Documents/Master/` and `Documents/Slave/`.
- Visual-check CSV/Markdown files named
  `Documents/schematic_visual_check_sheet_*.csv` and `.md`.
- `Documents/schematic_visual_check_review_status.md`
...
## Visual-Check / Pin-Review Status

The newer visual-check files are now the preferred handoff source before any
future schematic-led RTL change. They are reviewer-facing spreadsheets/checklists
and may be more current than older narrative audit rows.

Column convention:

- `Active Low = Y` only for active-low input/control pins.
- `Active Low = N` for non-active-low input/control pins.
- `Active Low = N.A.` for outputs, power pins, tied rails, and grouped rows.

Current key files:

| Sheet | Visual-check files | Current status |
| --- | --- | --- |
| `2A` | `schematic_visual_check_sheet_2a.csv` / `.md` | CPU has 40 pin rows. `8F`/`9E` LS139 decoders are expanded per-pin and `8F` `MORAM` is captured. The CPU `R/W` write-strobe path is expanded through `4/5E`, `7D`, and `7C` to document `PABR/W`, `BWRITE`, and `PAWRITE` generation. Other glue remains grouped and must be expanded from focused crops before implementation. |
| `3B` | `schematic_visual_check_sheet_3b.csv` / `.md` | Expanded from grouped rows; many labels remain `Check`. |
| `4A` | `schematic_visual_check_sheet_4a.csv` / `.md` | Motion-source inventory exists; exact RAM/ROM/shift-register pin confidence still needs review. |
| `4B` | `schematic_visual_check_sheet_4b.csv` / `.md` / `.numbers` | Per-pin checklist exists, but user has active uncommitted CSV/Numbers edits. Do not overwrite; use user-reviewed spreadsheet before implementation. |
| `5A` | `schematic_visual_check_sheet_5a.csv` / `.md` | Input buffers, color RAM/write gate, blanking, RGB latches, and RGB output are expanded. Color latch bit routing remains partly `Check`. |
| `6B` | `schematic_visual_check_sheet_6b.csv` / `.md` | Slave CPU, address/data buffers, interconnect, ROM/RAM/custom decode, LDX/LDY, and graph-control glue are expanded. Several small decode/glue rows remain `Check`. |
| `7A` | `schematic_visual_check_sheet_7a.csv` / `.md` | Sync-chain area expanded per-pin through custom `3H`; Slave Program ROM, Working RAM, and Communication RAM Interconnect are also expanded. ROM bank is represented as seven visible 2764 columns `1E/1F`, `1F/1H`, `1J`, `1K`, `1L`, `1M`, and `1N`. |

Cross-sheet timing note:

- The former `ESM/BSM` reading has been corrected to `E5M/B5M`.
- Current trace is `5MHZ -> Sheet 6B 3D LS244 pin 11 -> 3D pin 9 E5M
  -> J17 pin 31 -> Sheet 2A 1R pins 2/17 -> B5M*/B5M`.
- For FPGA implementation, the LS244/LS04 distribution does not necessarily
  need gate-for-gate modelling, but the naming and phase provenance matter.
