...
always_ff @(posedge clk) begin
    if (reset) begin
        ...
        // Start slightly before blanking; sprite rendering needs 64 sprites x
        // 4 fetches plus one ROM latency stage before the next scanline.
        if (ce_5m && hcnt == 9'd252) begin
            render_active <= 1;
            render_sprite <= 63;
            render_pair <= 0;
            render_pending <= 0;
